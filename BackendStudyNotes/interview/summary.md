...
* Redis: C语言开发的一个开源的（遵从BSD协议）高性能键值对（key-value）的内存数据库，可以用作数据库、缓存、消息中间件等。
    ...
    * 数据结构：String，Hash，Set，Sorted Set（跳跃表 skipList），List
      (一)String （SDS: simple dynamic string）
...
* RPC（Remote Procedure Call）—远程过程调用，它是一种通过网络从远程计算机程序上请求服务，而不需要了解底层网络技术的协议。比如两个不同的服务 A、B 部署在两台不同的机器上，那么服务 A 如果想要调用服务 B 中的某个方法该怎么办呢？使用 HTTP请求 当然可以，但是可能会比较慢而且一些优化做的并不好。 RPC 的出现就是为了解决这个问题。
  ...
  * RPC 框架的思路
    ![Alt text](./images/rpc.png?raw=true)
...
* ZooKeeper: https://www.zhihu.com/question/65852003/answer/656091418
  * ZooKeeper主要服务于分布式系统，可以用ZooKeeper来做：
    1. 统一配置管理:我们可以将common.yml这份配置放在ZooKeeper的Znode节点中，系统A、B、C监听着这个Znode节点有无变更，如果变更了，及时响应
    2. 统一命名服务:统一命名服务的理解其实跟域名一样，是我们为这某一部分的资源给它取一个名字，别人通过这个名字就可以拿到对应的资源。
    3. 分布式锁:访问的时候会创建带顺序号的临时/短暂(EPHEMERAL_SEQUENTIAL)节点,然后判断自己创建的是不是最小的那个节点 如果是，则拿到锁。 释放锁：执行完操作后，把创建的节点给删掉。如果不是，则监听比自己要小1的节点变化
    4. 集群管理。系统A、B、C为例，在ZooKeeper中创建临时节点, 监听这些临时节点，如果服务挂了，其他服务也会被感知
       * 除了能够感知节点的上下线变化，ZooKeeper还可以实现动态选举Master的功能。
       * (如果集群是主从架构模式下)原理也很简单，如果想要实现动态选举Master的功能，Znode节点的类型是带顺序号的临时节点(EPHEMERAL_SEQUENTIAL)就好了。
       * Zookeeper会每次选举最小编号的作为Master，如果Master挂了，自然对应的Znode节点就会删除。然后让新的最小编号作为Master，这样就可以实现动态选举的功能了。
  * 使用分布式系统就无法避免对节点管理的问题(需要实时感知节点的状态、对节点进行统一管理等等)，而由于这些问题处理起来可能相对麻烦和提高了系统的复杂性，ZooKeeper作为一个能够通用解决这些问题的中间件就应运而生了。
  * zookeeper结构
    ![Alt text](./images/zookeeper.png?raw=true)
...
* Socket vs web socket vs http vs https
  * Socket:
    * 它往往指的是TCP/IP网络环境中的两个连接端.
    * 通常所说的Socket API，是指操作系统中（也可能不是操作系统）提供的对于传输层（TCP/UDP）抽象的接口  
  * WebSocket
    * HTTP和WebSocket都是客户端-服务器通信中使用的通信协议
    ...
    * 它是一个有状态协议，这意味着客户端和服务器之间的连接将保持活动状态，直到被任何一方（客户端或服务器）终止  
  * HTTP协议
    * HTTP是一个基于TCP/IP通信协议来传递数据的协议，传输的数据类型为HTML 文件,、图片文件, 查询结果等。
    * HTTP协议一般用于B/S架构, HTTP是单向的，客户端发送请求，服务器发送响应
    * HTTP特点
    ...
    * 请求报文构成
      ...
      2. 请求头(Request Header) （User-Agent:os和浏览器类型, Referer：请求网址来源, Authentication: 验证）
      3. 请求正文  
    * 响应报文构成
      1. 状态行
      2. 响应头 (set-cookie,,)
  ...
  * HTTPS
    ...
    * 服务器端会将公钥发给CA，CA会产生一个安全证书，证书里面就包含服务器的公钥 -> 客户段通过证书获得网站的公匙 -> 生成对称密钥 -> 公匙加密对称密钥发给服务端 -> 对称密钥 加密明文发给服务端 
      * 如何验证CA证书的安全性
        * 浏览器内置CA的根证书：
          1. 证书颁发的机构是伪造的：浏览器不认识，直接认为是危险证书
          2. 证书颁发的机构是确实存在的，于是根据CA名，找到对应内置的CA根证书、CA的公钥。
          3. 用CA的公钥，对伪造的证书的摘要进行解密，发现解不了。认为是危险证书
          4. 假设代理通过某种途径，拿到XX的证书，然后将证书的公钥偷偷修改成自己的，然后喜滋滋的认为用户要上钩了。然而太单纯了：
...
* Java NIO：https://zhuanlan.zhihu.com/p/369062109
  * Java 序列化:Java序列化的本质是将Java对象保存为二进制字节码
     * Java 对象本质上是 class 字节码，服务器并不能根据这个字节码识别出该 Java 对象。所以，要提供一个公共的格式，不仅 Windows 能识别，你的服务器也能识别的公共的格式。
     * 我们将 Java 对象转换成公共的格式叫做序列化，将公共的格式转换成对象叫做反序列化
     * serialVersionUID是序列化前后的唯一标识符, 默认如果没有人为显式定义过serialVersionUID，那编译器会为它自动声明一个！
     * 凡是被static修饰的字段是不会被序列化的,凡是被transient修饰符修饰的字段也是不会被序列化的
     * Externalizable 接口:是Serializable接口的子类，用户要实现的writeExternal()和readExternal() 方法，用来决定如何序列化和反序列化。
  ...
  * Reactor: https://www.zhihu.com/question/26943938/answer/1856426252
...
* 从输入 URL 到页面展示到底发生了什么
  1、输入地址
     * 浏览器会从历史记录，书签等地方，找到已经输入的字符串可能对应的 url，然后给出智能提示，让你可以补全url地址。
  2、浏览器查找域名的 IP 地址
     * 览器首先要做的事情就是解析这个域名，一般来说，浏览器会首先查看本地硬盘的 hosts 文件，看看其中有没有和这个域名对应的规则，如果有的话就直接使用 hosts 文件里面的 ip 地址。
     * 如果在本地的 hosts 文件没有能够找到对应的 ip 地址，浏览器会发出一个 DNS请求到本地DNS服务器 。本地DNS服务器一般都是你的网络接入服务器商提供，比如中国电信
     * 本地DNS服务器会首先查询它的缓存记录，如果缓存中有此条记录，就可以直接返回结果，此过程是递归的方式进行查询。如果没有，本地DNS服务器还要向DNS根服务器进行查询。
     * 根DNS服务器没有记录具体的域名和IP地址的对应关系，而是告诉本地DNS服务器，你可以到域服务器上去继续查询，并给出域服务器的地址。
     * 本地DNS服务器继续向域服务器发出请求 在这个例子中，请求的对象是.com域服务器。.com域服务器收到请求之后，也不会直接返回域名和IP地址的对应关系，而是告诉本地DNS服务器，你的域名的解析服务器的地址。
     * 最后，本地DNS服务器向域名的解析服务器发出请求，这时就能收到一个域名和IP地址对应关系，本地DNS服务器不仅要把IP地址返回给用户电脑，还要把这个对应关系保存在缓存中
  ...
  4、服务器的永久重定向响应
     ...
     * 如果一个页面有两个地址 索引擎会认为它们是两个网站，结果造成每个搜索链接都减少从而降低排名
     * 减少缓存的域名数量
  6、服务器处理请求
     * Nginx负载均衡和反向代理  
  7、服务器返回一个 HTTP 响应
...
* 分布式事务：https://www.cnblogs.com/throwable/p/13263554.html
  * 业界目前主流的分布式事务解决方案主要有：多阶段提交方案（2PC、3PC）、补偿事务（TCC）和消息事务（主要是RocketMQ，基本思想也是多阶段提交方案，并且基于中间提供件轮询和重试，其他消息队列中间件并没有实现分布式事务）。这些方案的原理在此处不展开，目前网络中相应资料比较多，小结一下它们的特点：
    * https://juejin.cn/post/6844903936718012430
    1. 多阶段提交方案：常见的有二阶段和三阶段提交事务，需要额外的资源管理器来协调事务，数据一致性强，但是实现方案比较复杂，对性能的牺牲比较大（主要是需要对资源锁定，等待所有事务提交才能解锁），不适用于高并发的场景，目前比较知名的有阿里开源的fescar。
       * 再来看一下两阶段提交的两个“不归路”时刻，这两个承诺保证了两阶段提交的原子性。https://zhuanlan.zhihu.com/p/76209860
         1. 当参与者回复“可以提交”时，它承诺稍后肯定提交。
         2. 当协调者做出决定后，这一决定不可撤销。
       * 三阶段提交解决了两阶段提交的协调者失效问题，但不能保证原子性。它做出如下改动：
         1. 参与者也可以应对超时（两阶段提交只有协调者有超时机制，三阶段提交就都有了）。
         2. 把准备阶段再次一分为二。 
    ...
    4. 本地消息表对于本地消息队列来说，核心就是将大事务转变为小事务，还是用上面下订单扣库存的例子说说明
